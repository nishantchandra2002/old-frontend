$(document).ready(function () {
  ...
  window.pixelarity = {
    ...
    open: function (imgObj, square, callback, imageType, imageQuality) {
      ...
      this.imageQuality =
        Number(imageQuality) > 0 && Number(imageQuality) <= 1
          ? Number(imageQuality)
          : 1;
      ...
      if (
        ...
        //JPG and any other would default to JPEG//
      ...
      var getType = {};
      this.callback =
        callback && getType.toString.call(callback) === '[object Function]'
          ? callback
          : false;
      ...
      $(img).on('load', function () {
        if (img.width > img.height) {
          ...
          that.can.style.height =
            img.height * ((($(window).width() / 2) * 1) / img.width) + 'px';
          ...
          pixelarity.selectionBox.css({
            left: $(window).width() / 2 - $(that.can).height() / 2 + 10 + 'px',
            top: $(window).height() / 2 - $(that.can).height() / 2 - 15 + 'px'
          });
        } else if (img.width < img.height) {
          that.can.width = img.width;
          that.can.height = img.height;

          that.can.style.width =
            img.width * ((($(window).height() / 3) * 2) / img.height) + 'px';
          ...
          pixelarity.selectionBox.css({
            left: $(window).width() / 2 - $(that.can).width() / 2 + 10 + 'px',
            top: $(window).height() / 2 - $(that.can).width() / 2 - 15 + 'px'
          });
        } else {
          that.can.width = img.width;
          that.can.height = img.height;
    ...
    changeFilter: function (filter) {
      ...
      $('#pixelarity-filter-opt-' + filter).addClass(
      ...
      this.ctx.drawImage(
        pixelarity.processCan,
        0,
        0,
        pixelarity.can.width,
        pixelarity.can.height
      );
    },
  ...
  pixelarity.selectionBox.on('mousedown', function (e) {
    ...
    if (
      pixelarity.selectionBox.width() - rcx <= 28 &&
      pixelarity.selectionBox.height() - rcy <= 28
    ) {
      pixelarity.drag = false;
  ...
  $(pixelarity.can).on('mousedown', function (e) {
    if (pixelarity.tool == 'draw') {
      ...
      var ratio = pixelarity.can.width / $(pixelarity.can).width();
      pixelarity.lastDrawX =
        (e.clientX - windowOffset($(pixelarity.can)).left) * ratio;
      pixelarity.lastDrawY =
        (e.clientY - windowOffset($(pixelarity.can)).top) * ratio;
    }
  });
  ...
  $(document).on('mousemove', function (e) {
    ...
    if (pixelarity.drag === true && pixelarity.status) {
      ...
      if (
        xMove + pixelarity.selectionBox.width() >=
          $(pixelarity.can).width() + windowOffset($(pixelarity.can)).left ||
        xMove <= windowOffset($(pixelarity.can)).left
      ) {
        if (xMove <= windowOffset($(pixelarity.can)).left) {
          pixelarity.selectionBox.css({
            left: windowOffset($(pixelarity.can)).left + 'px'
          });
        } else {
          pixelarity.selectionBox.css({
            left:
              windowOffset($(pixelarity.can)).left +
              $(pixelarity.can).width() -
              pixelarity.selectionBox.width() +
              'px'
          });
        }
      } else {
      ...
      if (
        yMove + pixelarity.selectionBox.height() >=
          $(pixelarity.can).height() + windowOffset($(pixelarity.can)).top ||
        yMove <= windowOffset($(pixelarity.can)).top
      ) {
        if (yMove <= windowOffset($(pixelarity.can)).top) {
          pixelarity.selectionBox.css({
            top: windowOffset($(pixelarity.can)).top + 'px'
          });
        } else {
          pixelarity.selectionBox.css({
            top:
              windowOffset($(pixelarity.can)).top +
              $(pixelarity.can).height() -
              pixelarity.selectionBox.height() +
              'px'
          });
        }
      } else {
      ...
      if (pixelarity.square) {
        if (nWidth >= nHeight) {
          //Width is the dominating dimension;
          nHeight = nWidth;
          if (nWidth < 100) {
            nWidth = 100;
            nHeight = 100;
        ...
        if (
          nWidth + windowOffset(pixelarity.selectionBox).left >=
          $(pixelarity.can).width() + windowOffset($(pixelarity.can)).left
        ) {
          nWidth =
            windowOffset($(pixelarity.can)).left +
            $(pixelarity.can).width() -
            windowOffset(pixelarity.selectionBox).left;
          if (
            windowOffset(pixelarity.selectionBox).top + nWidth >
            $(pixelarity.can).height() + windowOffset($(pixelarity.can)).top
          ) {
            nWidth =
              windowOffset($(pixelarity.can)).top +
              $(pixelarity.can).height() -
              windowOffset(pixelarity.selectionBox).top;
          }
          nHeight = nWidth;
        } else if (
          nHeight + windowOffset(pixelarity.selectionBox).top >=
          $(pixelarity.can).height() + windowOffset($(pixelarity.can)).top
        ) {
          nHeight =
            windowOffset($(pixelarity.can)).top +
            ...
            windowOffset(pixelarity.selectionBox).top;
          if (
            windowOffset(pixelarity.selectionBox).left + nHeight >
            $(pixelarity.can).width() + windowOffset($(pixelarity.can)).left
          ) {
            nHeight =
              windowOffset($(pixelarity.can)).left +
        ...
        if (
          ...
          //REASON: nWidth + windowOffset(pixelarity.selectionBox).left = e.clientX;
          nWidth =
            windowOffset($(pixelarity.can)).left +
            $(pixelarity.can).width() -
            windowOffset(pixelarity.selectionBox).left;
      ...
      var xMove = (x - pixelarity.lastDrawX) / n,
        yMove = (y - pixelarity.lastDrawY) / n;
  ...
  pixelarity.saveBtn.on('click', function () {
    ...
    pixelarity.callback(
      pixelarity.processCan.toDataURL(
        'image/' + pixelarity.imageType,
        pixelarity.imageQuality
      )
    );
    pixelarity.close();
  });
  ...
  $(window).on('resize', function () {
    if (pixelarity.status) {
      pixelarity.selectionBox.css({
        left:
          $(window).width() / 2 - $(pixelarity.can).height() / 2 + 10 + 'px',
        top: $(window).height() / 2 - $(pixelarity.can).height() / 2 + 10 + 'px'
      });
    }
  });
